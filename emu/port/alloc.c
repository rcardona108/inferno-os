...
dopoolalloc(Pool *p, ulong asize, ulong pc)
{
	...
	/* Align allocations to 16 bytes */
	{
		...
		l = (Bhdr *)(((ulong)t + align) & ~align);
	}
#else
	/* Double alignment */
	l = (Bhdr *)(((ulong)l + 7) & ~7);
#endif
	if(p->chain != nil && (char*)l-(char*)B2LIMIT(p->chain)-ldr == 0){
		/* can merge chains */
	...
	l->magic = MAGIC_A;		/* Make the block we are going to return */
	l->size = size;
	B2T(l)->hdr = l;
	q = l;
